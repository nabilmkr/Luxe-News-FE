...
import { getCategoryNewsDetail } from '../api/categoryService';
...
const NewsDetail = () => {
  ...
  useEffect(() => {
    const fetchNewsDetail = async () => {
      ...
      try {
        if (slug) {
          // First try the regular news endpoint
          try {
          ...
          } catch (regularNewsError) {
            console.log('Regular news not found, trying category news endpoint');
          }
          
          // If regular news fails, try category news endpoint
          try {
            const categoryResponse = await getCategoryNewsDetail(slug);
            
            if (categoryResponse.status === 'success') {
              setNews(categoryResponse.data);
              setComments(categoryResponse.data.comments || []);
              return; // Exit if successful
            }
          } catch (categoryNewsError) {
            console.error('Error fetching from category news:', categoryNewsError);
            throw new Error('Berita tidak ditemukan');
          }
        } else {
          throw new Error('Parameter berita tidak valid');
        }
      } catch (err) {
  ...
  // Determine image source based on news source
  const determineImageSrc = (news) => {
    if (!news || (!news.image && !news.thumbnail)) return 'https://placehold.co/800x400?text=No+Image';
    
    const imageString = news.thumbnail || news.image;
    
    // Log untuk debugging
    console.log('Original image path:', imageString);
    
    // Jika URL absolut (dimulai dengan http atau https)
    ...
    if (imageString.startsWith('/storage')) {
      return `${baseUrl}${imageString}`;
    } else if (imageString.startsWith('storage/')) {
      return `${baseUrl}/${imageString}`;
    } else {
      return `${baseUrl}/storage/${imageString}`;
    }
  };
  
  const imageSrc = determineImageSrc(news);
  
  // Tambahkan error handling untuk debugging gambar
  const handleImageError = (e) => {
    console.error('Image failed to load:', e.target.src);
    e.target.src = 'https://placehold.co/800x400?text=No+Image';
    e.target.onerror = null; // Hindari loop jika placeholder juga gagal
  };
        ...
        <div className="max-w-4xl mx-auto bg-gray-800 rounded-lg overflow-hidden">
            ...
            <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black to-transparent p-6">
              <div className="flex items-center text-sm text-white/80 mb-2">
                <span className="bg-indigo-600 px-2 py-1 rounded mr-3">
                  {news.category?.name || news.categoryName || 'Umum'}
                </span>
                <span>{new Date(news.published_at || news.date).toLocaleDateString('id-ID')}</span>
                {(news.game || news.gameName) && (
                  <span className="ml-3 px-2 py-1 bg-gray-700 rounded">{news.game?.name || news.gameName}</span>
                )}
              </div>
            </div>
          </div>
          ...
          <div className="p-8">
            ...
            <div className="prose prose-lg prose-invert max-w-none">
              {typeof news.content === 'string' && news.content.includes('<') ? (
