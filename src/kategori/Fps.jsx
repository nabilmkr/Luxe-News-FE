...
import Carouselfps from "../components/design/Carouselfps";
...
const Fps = () => {
  ...
  useEffect(() => {
    const fetchNews = async () => {
      try {
        setLoading(true);
        // Selalu kirim "FPS" sebagai kategori ke backend
        const response = await getCategoryNews("FPS", searchTerm);
  ...
  useEffect(() => {
    if (location.state?.selectedGame) {
      const gameCategory = news.find(
        item => item.category.toLowerCase() === location.state.selectedGame.toLowerCase()
      )?.category || "all";
  ...
  return (
    <Section id="fps">
      <div className="relative z-1 max-w-[62rem] mx-auto text-center mb-[3.875rem] md:mb-20 lg:mb-[6.25rem]">
        <h1 className="h1 mb-6">FPS Games</h1>
        <p className="body-1 max-w-3xl mx-auto mb-6 text-n-2 lg:mb-8">
          Kecepatan, akurasi, dan adrenalin—itulah dunia FPS! Temukan semua berita hangat seputar game seperti Valorant, Call of Duty, Counter-Strike 2, dan lainnya.
        </p>
      </div>
